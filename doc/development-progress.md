...
- 状态：已完成
- 范围：完成模块 11 中“任务 06：补本地验收脚本、README 与运行说明”，为第一阶段 `Step 10` QA 回答链路固化真实本地验收入口
- 结果：
  - 已新增：
    - `scripts/verify_local_step10_answer_generation.py`
  - 当前本地验收脚本已覆盖：
    - 单文件导入
    - `Step 09` 全链路：
      - query decomposition
      ...
      - context builder
      - citation payload
    - `Step 10` 第一阶段 QA 回答生成
    - 本地空上下文无答案兜底
  - 已同步更新：
    - `README.md`
    - `scripts/README.md`
  - 当前验收口径已收敛为：
    - 真实 `Step 10` 调用返回“带 citation 的结构化回答”视为通过
    - 若当前本地知识库或网关在严格协议下保守拒答，返回标准化无答案结果也视为通过
  - 已完成一次真实本地脚本执行
  - 当前真实本地返回已确认：
    - 导入成功
    ...
    - `citation_count = 6`
    - 真实回答链路返回了标准化低置信度无答案结果
    - 本地空上下文兜底返回正常
- 影响：
  - 模块 11 已具备独立的真实本地验收入口
  - `Step 10` 第一阶段问答链路的运行说明已对外可见
- 遗留问题：
  - 当前真实网关在严格结构化协议下偏保守，隔离证据用例也可能返回标准拒答
  - 若后续要提升“正常 cited answer”命中率，需要进入新的 prompt / generation 稳定性优化任务
- 下一步：
  - 模块 11 已完成，可进入下一个开发模块讨论

#### 记录 079：完成模块 11 任务 05，补无答案场景与生成降级路径

- 状态：已完成
- 范围：完成模块 11 中“任务 05：实现无答案场景与生成降级路径”，为第一阶段 QA 回答链路补齐拒答规范与最小降级出口
- 结果：
  - 已在 `answer_generation_service` 中补齐第一阶段无答案场景：
    - `empty_context`
    - `empty_citations`
    - 模型判定的 `insufficient_evidence`
    - 模型判定的 `conflicting_evidence`
  - 当前实现边界已收敛为：
    - 空 `context` 由应用层直接返回标准无答案结果
    - 空 `citations` 由应用层直接返回标准无答案结果
    - “证据不足 / 证据冲突” 不做复杂本地规则识别，交由 prompt 约束模型判断
  - 已补标准化无答案文案：
    - `当前知识库中没有检索到可用于回答该问题的相关内容。`
    - `当前检索到的知识不足以可靠回答这个问题。`
  ...
  - 已补生成降级路径：
    ...
    - 当 `confidence` 非法
    - 当 `citation_id` 映射失败
    - 当前统一降级为低置信度结构化无答案结果，而不是直接抛异常
  - 已加强 prompt 约束：
    - 证据不足时必须返回标准拒答文案
    - 证据冲突时必须返回标准拒答文案
    - 返回无答案时 `sources=[]` 且 `confidence=low`
  - 已新增单元测试覆盖：
    - 空 `context`
    - 空 `citations`
    - 模型判定证据不足
    ...
    - 未知 `citation_id` 降级
- 影响：
  - 当前第一阶段 QA 回答链路已具备稳定拒答出口
  - 避免将生成失败或来源映射异常直接暴露成调用错误
- 遗留问题：
  - 当前仍未实现更细粒度的冲突检测与证据强弱规则判断
  - 当前降级仍统一返回无答案，而不是切换到检索摘要模式
- 下一步：
  - 继续推进模块 11 任务 06：补本地验收脚本、README 与运行说明

#### 记录 078：完成模块 11 任务 02-04，落地第一版 QA 回答生成链路

- 状态：已完成
- 范围：完成模块 11 中“任务 02：为第一阶段问答任务建立 prompt 模板”“任务 03：建立统一结构化回答协议”“任务 04：实现引用插入与回答层映射规则”，在现有 `context + citation + llm` 底座上补齐第一版 QA 回答生成链路
- 结果：
  - 已新增：
    - `src/mindwiki/application/answer_generation_service.py`
  - 已补充回答结果模型：
    - `AnswerGenerationResult`
    - `ANSWER_CONFIDENCE_VALUES`
  - 当前第一版回答生成链路已实现：
    - 基于 `ContextBuildResult + CitationBuildResult` 组装 QA prompt
    - 通过 `GenerateTextInput` 调用现有 `generate_text`
    - 约束结构化输出包含：
      - `answer`
      - `sources`
      - `confidence`
    - 对 `confidence` 执行枚举校验：
      ...
      - `low`
  - 当前引用映射规则已收敛为：
    - 模型侧 `sources` 只返回 `citation_id`
    - 应用侧再映射回现有 `CitationPayload`
    - 最终回答结果继续直接复用现有 citation payload 结构
  - 当前最小无答案兜底已接入：
    - 当 `context` 或 `citations` 为空时，不走模型调用，直接返回低置信度无答案结果
  - 已新增单元测试覆盖：
    - 正常回答生成
    - citation id 去重与映射
    - 空 context 本地无答案返回
    ...
    - 未知 `citation_id` 拒绝
- 影响：
  - 项目第一次从“只能产出 context 和 citations”推进到“可以产出带来源的结构化回答”
  - 为后续模块 11 的无答案细化、降级路径和本地验收脚本提供直接承接点
- 遗留问题：
  - 当前模型侧 `sources` 仍是最小 `citation_id` 引用，而不是完整 citation 对象回填
  - 无答案场景目前只覆盖空 `context / citations` 的本地兜底，尚未细化到冲突证据与生成降级
  - 尚未补真实本地验收脚本
- 下一步：
  - 继续推进模块 11 任务 05：实现无答案场景与生成降级路径

#### 记录 077：完成模块 11 任务 01，生成能力设计对接与当前代码差异修正

- 状态：已完成
- 范围：完成模块 11 中“任务 01：生成能力设计对接与当前代码差异修正”，对照 `Step 10.1-10.6` 与当前代码基础，收敛第一阶段问答能力边界
- 结果：
  - 已确认当前模块第一阶段只进入：
    - 基于知识片段问答
  - 已确认当前模块第一阶段暂不进入：
  ...
  - 已确认当前第一阶段回答协议先收敛为：
    ...
    - `sources`
    - `confidence`
  - 已确认当前第一阶段回答层与现有代码的承接方式：
    - 继续复用现有 `ContextBuildResult`
    - 继续复用现有 `CitationPayload`
    - `confidence` 第一阶段由模型输出，系统只做枚举校验
  - 已修正当前差异判断：
    - 当前代码已有 `context builder + citation payload + generate_text`
    - 当前代码缺少 `Step 10` 的 prompt 模板、回答协议、来源映射和无答案规范
- 影响：
  - 模块 11 的实施边界已从泛化“生成能力”收敛为可直接落地的 QA 子链路
  - 避免在当前阶段同时引入摘要、专题归纳和推荐，导致范围再次扩散
- 下一步：
  - 开始落地任务 02-04：先建立 QA prompt、结构化协议与 citation 映射

#### 记录 076：启动新开发模块，按 `Step 10.1-10.6` 推进回答生成链路

- 状态：进行中
- 范围：在模块 10 已完成 `Step 09.4-9.6` 检索编排后半段闭环并通过真实本地验收的基础上，启动下一开发模块，继续推进 `Step 10` 回答生成链路
- 模块定位：
  - 当前新模块正式进入：
    - 回答生成
    - 回答约束
    ...
    - 无答案场景
    - 生成降级路径
  - 目标是让系统第一次具备“基于已构建 context 和 citation 产出稳定回答”的能力
- 启动原因：
  - 当前系统已经具备：
    - query decomposition
    - query expansion
  ...
  - 当前系统仍缺少：
    ...
    - 无答案输出规范
    - 生成降级路径
  - 若此时不进入 `Step 10`，项目仍停留在“能找证据”而不是“能基于证据回答”
  - 已进一步确认当前模块第一阶段正式收敛为：
    - 只做：
      - 基于知识片段问答
    - 暂不进入：
      ...
      - 历史观点对比
      - 相关笔记推荐
  - 已进一步确认当前第一阶段回答协议先收敛为：
    - `answer`
    - `sources`
    - `confidence`
  - 已进一步确认：
    - `sources` 先直接复用现有 citation payload
    - `confidence` 第一阶段由模型输出，系统只做枚举校验
- 分步任务拆解：
  - 任务 01：生成能力设计对接与当前代码差异修正
    - 对照 `Step 10.1-10.6` 与当前实现完成第一轮差异梳理
    - 明确哪些生成任务进入当前模块，哪些留到后续迭代
  - 任务 02：为第一阶段问答任务建立 prompt 模板
    - 先聚焦：
      - 基于知识片段问答
    - 暂不同时并入摘要、专题归纳、对比分析的完整实现
  - 任务 03：建立统一结构化回答协议
    - 至少包含：
      - `answer`
      - `sources`
    ...
    - 根据当前需要决定是否同时保留：
      - `related_notes`
  - 任务 04：实现引用插入与回答层映射规则
    - 让回答内容与 citation payload 可以稳定对应
  - 任务 05：实现无答案场景与生成降级路径
    - 覆盖：
      - 证据不足
      - 召回为空
      - 上下文冲突
      - 仅返回检索摘要的降级模式
  - 任务 06：补本地验收脚本、README 与运行说明
    - 固化 `Step 10` 第一阶段问答能力的真实本地验收入口
- 当前边界判断：
  - 当前模块优先目标是：
    - 基于现有 context + citations 生成结构化问答结果
  - 当前模块不应提前进入：
    - 前端问答界面
    - 评估闭环
    - 多任务类型一次性全量实现
- 当前建议执行顺序：
  - 先完成 `Step 10` 与当前实现差异对接
  - 再补问答 prompt 模板
  - 然后补统一回答协议
  - 再补引用插入规则
  - 最后补无答案 / 降级与本地验收
- 遗留问题：
  - 第一阶段是否只做问答，不同时并入摘要 / 主题归纳 / 对比分析，需要在任务 01 中正式收口
  - `sources` 字段是直接引用 `CitationPayload` 还是再包一层回答态结构，需在任务 03-04 中确认
  - `confidence` 的第一阶段来源是规则推断还是模型显式输出，需在任务 03 中收敛
- 下一步：
  - 开始当前模块任务 01：生成能力设计对接与当前代码差异修正
...
### 模块 11：`Step 10.1-10.6` 回答生成链路

| 任务 | 内容 | 状态 | 备注 |
| --- | --- | --- | --- |
| 01 | 生成能力设计对接与当前代码差异修正 | 已完成 | 已收敛第一阶段只做 QA，协议先落 `answer / sources / confidence` |
| 02 | 为第一阶段问答任务建立 prompt 模板 | 已完成 | 已落第一版 QA prompt，输入承接 `context + citations` |
| 03 | 建立统一结构化回答协议 | 已完成 | 已新增回答结果模型并固化 `answer / sources / confidence` |
