...
- `Chunk` 的 embedding 及向量检索索引进入 Qdrant
- `Chunk` 的可检索文本应进入 PostgreSQL FTS
...
- Qdrant 中只存放检索所需最小必要信息
- 业务真相仍然以 PostgreSQL 为主
- 不把 Qdrant 当作主数据源

## 哪些数据进入 PostgreSQL FTS
...
Qdrant 只承担向量召回职责，不承担主数据职责。

### 3. PostgreSQL FTS 是字面检索层
...
第一阶段采用“PostgreSQL 作为主数据源、Qdrant 负责向量检索、PostgreSQL FTS 负责全文检索、本地文件系统负责原始文件”的多存储协同方案：

- 结构化数据归 PostgreSQL
- 语义检索归 Qdrant
- 关键词检索归 PostgreSQL FTS
- 原始文件归本地文件系统
